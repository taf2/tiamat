...
function launchServer(config) {
  ...
  function forkWorker(i) {
    if (config.before_fork) { try { config.before_fork(); } catch(e) { console.error(e); } }
    var pid = posix.fork();
    if (pid == 0) {
      posix.setsid();
      resetWorker();
      workerSignals();
      if (config.after_fork) { try { config.after_fork(); } catch(e) { console.error(e); } }
  ...
  function reexecuteMaster() {
    ...
    if (config.before_exec) { try { config.before_exec(); } catch(e) { console.error(e); } }
