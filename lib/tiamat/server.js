...
function launchServer(config) {
  ...
  function workerSetup(fd, server, config) {
    logw('info', "got sigquit with %d connections", server.connections);
  ...
  function runWorker(fd,id) {
    try {
      ...
      logw('info', "alive with parent(%d) at %s", posix.getppid(), config.worker_app);
      ...
      logw('info', "app loaded!");
      if (typeof(app.run) == 'function') {
        app.run(config, function(server) { workerSetup(fd, server, config); });
      }
      else if (typeof(app.listenFD) == 'function') {
        workerSetup(fd, app, config);
      }
      else {
  ...
  function maintainWorkerCount() {
    ...
    logm('info', "worker count difference: %d", off);
    if (!off) { return; }
  ...
  function startMissingWorkers() {
    logm('info', "start missing workers");
    var pid;
